...
public class enemy : MonoBehaviour
{
    ...
    void Follow(Vector2 horizontal, Vector2 vertical)
    {
        ...
        // (2) 공격 시작/해제 (원본 로직 유지)
        if (horizontal.magnitude <= stop_dist && -1 <= vertical.y && vertical.y < 1)
        {
            attacking = true;
        }
        else if (attacking && (!(hor.magnitude <= agrowexitdis) || vertical.y > 2.21 || vertical.y < -1 || (vertical.y > 1 && playerHealth.isGround == true)))
        {
            attacking = false;
            attackdelay = 0;
        }
