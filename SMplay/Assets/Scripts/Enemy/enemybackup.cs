...
public class enemybackup : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (debugAI)
        {
            if (!debugWarnedMasks && (groundMask.value == 0 || wallMask.value == 0))
            {
                debugWarnedMasks = true;
                Debug.LogWarning($"[enemy:{name}] groundMask/wallMask가 비어있음(0). 프리팹 Inspector에서 Ground 레이어 체크 필요!");
            }
            ...
            if (debugTimer >= debugInterval)
            {
                ...
                Debug.Log(
                    ...
                    $"masks g={groundMask.value} w={wallMask.value} shotDelay={shotdelay}"
                );
            }
        }
